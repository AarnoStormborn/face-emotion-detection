...
class VideoInference:
    ...
    def draw_bounding_boxes(self, fps):
        for face in self.faces:
            x1, y1, x2, y2 = face
            cv2.rectangle(self.frame, (int(x1),int(y1)), (int(x2),int(y2)), color=(0,255,0), thickness=3)
            cv2.putText(self.frame, f"{self.emotion}", (int(x1), int(y1-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
            cv2.putText(self.frame, f'FPS: {fps}', (50,75), cv2.FONT_HERSHEY_SIMPLEX, 
    ...
    def inference(self):
        ...
        for self.frame in self.frames:
            ...
            self.nTime = time()
            try:
                fps = int(1 / (self.nTime - self.pTime))
            except ZeroDivisionError:
                fps = 0
            
            self.draw_bounding_boxes(fps)
